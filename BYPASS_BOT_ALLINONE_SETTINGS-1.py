...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8999579043:AAHUyJAti3F1C4h_k2wkAtZFcNZWD3oBOkw"   # ← ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "6153895991"))
...
def main_menu_kb(admin: bool = False) -> types.InlineKeyboardMarkup:
    """Keep the main menu focused only on actual hosting/bot operations."""
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['diamond']}  Mʏ Bᴏᴛꜱ", callback_data="menu_bots", style="primary"),
        Btn(f"{G['upload']}  Uᴘʟᴏᴀᴅ Bᴏᴛ", callback_data="menu_upload", style="success"),
    )
    kb.add(
        Btn(f"{G['rec']}  Hᴇʟᴘ", callback_data="menu_help", style="primary"),
        Btn(f"{G['broadcast']}  Sᴜᴘᴘᴏʀᴛ", callback_data="menu_support", style="primary"),
    )
    if admin:
        kb.add(Btn(f"{G['shield']}  Aᴅᴍɪɴ Pᴀɴᴇʟ", callback_data="menu_admin", style="danger"))
    return kb
...
def admin_kb() -> types.InlineKeyboardMarkup:
    ...
    kb.add(Btn(f"{G['upload']}  Mᴇɴᴜ Pʜᴏᴛᴏꜱ", callback_data="adm_photos", style="primary"))
...
def user_max_bots(u: Dict[str, Any]) -> int:
    # Owner hosting is unlimited; all other users keep existing plan limits.
    try:
        if is_owner(int(u.get("_id", 0))):
            return -1
    except Exception:
        pass
...
def action_bot_clone(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    max_bots = user_max_bots(u)
    if max_bots != -1 and len(list_user_bots(call.from_user.id)) >= max_bots:
...
def render_adm_settings(call: types.CallbackQuery) -> None:
    """ALLINONE-style Settings view; Bypass backend/admin panel remains unchanged."""
    uid = call.from_user.id
    if not admin_only_call(call, "view_stats"):
        return
    d = db_load()
    users = d.get("users", {})
    bots = d.get("bots", {})
    user = users.get(str(uid), users.get(uid, {})) or {}
    file_count = len(user.get("files", [])) if isinstance(user, dict) else 0
    running = sum(1 for x in RUNNING.values() if x.get("proc") and x["proc"].poll() is None)
    settings_text = (
        f"<b>⚙ YOUR SETTINGS</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if is_admin(uid):
        settings_text += (
            f"\n<b>ADMIN SETTINGS</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🤖 Bot Locked: {'Yes' if bool(get_setting('maintenance', False)) else 'No'}\n"
    ...
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("admin", ""), settings_text, kb, call=call)
...
def render_adm_import_export(call: types.CallbackQuery) -> None:
    ...
    kb.add(Btn("👥  Exᴘᴏʀᴛ Uꜱᴇʀꜱ CSV", callback_data="adm_user_export_csv", style="primary"))
    kb.add(
...
def on_document(m: types.Message) -> None:
    ...
    if not RATE.allow(uid):
        maybe_auto_ban(uid, "rate")
        return
    if not is_owner(uid) and not UPLOAD_RATE.allow(uid):
...
def _handle_bot_upload(m: types.Message) -> None:
    ...
    u = db_load()["users"][str(uid)]
    max_bots = user_max_bots(u)
    if max_bots != -1 and len(list_user_bots(uid)) >= max_bots:
